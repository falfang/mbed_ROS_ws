#ifndef _ROS_AKROS_bridge_msgs_motor_reply_timestamped_h
#define _ROS_AKROS_bridge_msgs_motor_reply_timestamped_h
...
namespace AKROS_bridge_msgs
{
  ...
  class motor_reply_timestamped : public ros::Msg
  {
    public:
      uint32_t motor_length;
      typedef AKROS_bridge_msgs::motor_reply_single _motor_type;
      _motor_type st_motor;
      _motor_type * motor;

    motor_reply_timestamped():
    ...
    virtual int deserialize(unsigned char *inbuffer) override
    {
      ...
      if(motor_lengthT > motor_length)
        this->motor = (AKROS_bridge_msgs::motor_reply_single*)realloc(this->motor, motor_lengthT * sizeof(AKROS_bridge_msgs::motor_reply_single));
      motor_length = motor_lengthT;
      for( uint32_t i = 0; i < motor_length; i++){
      offset += this->st_motor.deserialize(inbuffer + offset);
        memcpy( &(this->motor[i]), &(this->st_motor), sizeof(AKROS_bridge_msgs::motor_reply_single));
      }
     return offset;
    }

    virtual const char * getType() override { return "AKROS_bridge_msgs/motor_reply_timestamped"; };
    virtual const char * getMD5() override { return "85b42a19ebcd114be5917464f58e8729"; };
